...
int foodnumber,drinknumber=0;
String food[20],drink[20];
         ...
         if(ReadString.indexOf('?') >0)//checks for ?
          {
            ...
            kondisi = 1;
            if(makanan.indexOf('%') >0)//checks for ?
            {
              ind2 = makanan.indexOf('%');
              food[foodnumber]=makanan.substring(0,ind2);
              makanan=makanan.substring(ind2+1,makanan.length());
              foodnumber++;
            }
            if(minuman.indexOf('%') >0)//checks for ?
            {
              ind2 = minuman.indexOf('%');
              drink[drinknumber]=minuman.substring(0,ind2);
              minuman=minuman.substring(ind2+1,minuman.length());
              drinknumber++;
            }
...
"while (food[i]){\r\n"+
"if (foodnumber[i]>0){\r\n"+
"response += food[i]+'+'+foodnumber[i]+'+buah%';}i++;}\r\n"+
"i=0;\r\n"+
"response+='&Minuman=';\r\n"+
"while (drink[i]){\r\n"+
"if (drinknumber[i]>0){\r\n"+
"response += drink[i]+'+'+drinknumber[i]+'+gelas%';}i++;}\r\n"+
